...
January, 2021; for Professor Trash Wheel, from Januray, 2017 to January,
2021) – weight in tons, volume in cubic yards, amount of different types
of trash, such as plastic bottles, polystyrene, cigarette butts, sports
balls, etc.
...
`pivot_longer()`. Since the `party` is a binary indicator of the
president party (1 = yes, 0 = no), for me, those rows with `party` coded
as 0 can be filtered out to make the dataset cleaner.
...
and same ascending sequence as in the `pols` dataset.
...
The `pols` dataset contains 822 observations with 10 variables. It
...
containing 822 observations with 12 variables. It summarizes the
...
president / governor, closing values of S&P stock index, and
unemployment rate at the associated time point from January, 1947 to
June, 2015.
