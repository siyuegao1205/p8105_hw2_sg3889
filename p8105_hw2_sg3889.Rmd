...
The following code chunk aims to tidy the snp.csv by a similar process. Note that the `year` and `month` variables are kept in the same format and same ascending sequence as in the `pols_months` dataset.
...
The following code chunk aims to tidy the unemployment.csv to be merged with `pols_months` and `snp`. A `pivot_longer()` process will be necessary. Note that the `year` and `month` should be in the same format as those in the previous datasets.
